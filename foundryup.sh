...
main() {
  ...
  if [[ -n "$FOUNDRYUP_LOCAL_REPO" ]]; then
    ...
    ensure ln -s "$PWD/target/release/chisel" "$FOUNDRY_BIN_DIR/chisel"
  ...
  if [[ "$FOUNDRYUP_REPO" == "foundry-rs/foundry" && -z "$FOUNDRYUP_BRANCH" && -z "$FOUNDRYUP_COMMIT" ]]; then
    ...
    say "installed - $($FOUNDRY_BIN_DIR/chisel --version)"
    say "done"
    ...
    if [[ $(which chisel) =~ "cargo" ]]; then
      warn "it appears your system has already has chisel installed via cargo. you may need to run 'rm $(which chisel)' to allow foundryup to take precedence!"
    fi
  else
    need_cmd cargo
    ...
    # checkout submodules for Chisel build
    ...
    # install chisel
    RUSTFLAGS="-C target-cpu=native" ensure cargo install --path ./chisel --bin chisel --locked --force --root $FOUNDRY_DIR
    ...
    if command -v help2man &> /dev/null ; then
      ...
      help2man -N $FOUNDRY_BIN_DIR/chisel > $FOUNDRY_MAN_DIR/chisel.1
    fi
    say "done"
  fi
}
